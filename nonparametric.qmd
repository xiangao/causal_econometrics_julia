...
DataFrame(Estimand=["True ATE"], Value=[round(True_Psi, digits=4)])
```
...
DataFrame(Estimand=["ATE (plug-in)"], Value=[round(plug_in_ate, digits=4)])
```
...
DataFrame(Estimand=["ATE (AIPW)", "95% CI Lower", "95% CI Upper"],
          Value=round.(res_manual_aipw, digits=4))
```
...
println("\nTrue ATE: ", round(True_Psi, digits=4))
```
...
DataFrame(
    Estimand = ["DML PLR Estimate", "Std. Error", "95% CI Lower", "95% CI Upper"],
    Value    = round.([theta_hat, se_theta, ci_lb, ci_ub], digits=4)
)
```
