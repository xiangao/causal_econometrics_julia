...
This is why the cohort-FE requirement matters only for nonlinear (Poisson/logit) ETWFE: in the *linear* Gaussian examples above, unit FE and cohort FE give identical ATTs because the cohort effects are collinear with unit FE and the cohort×year interactions still identify the ATTs within cohort. The within-unit transform does not pass through the nonlinear link, so for Poisson/logit it must be cohort FE.
