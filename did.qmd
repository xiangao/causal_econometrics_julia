...
show(stdout, first(mpdta, 6); eltypes=false)
```
...
#| cache: true
#| echo: true
show(stdout, MIME("text/plain"), StatsAPI.coeftable(mod))
```
...
show(stdout, first(select(did_data, :id, :year, :y, :x, :d4, :d5, :d6, :te4, :te5, :te6, :first_treat), 6); eltypes=false)
```
