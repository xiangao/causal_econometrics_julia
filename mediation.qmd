...
function preview(df, n=6)
    df2 = copy(first(df, n))
    for col in names(df2)
        if eltype(df2[!, col]) == Bool
            df2[!, col] = Int.(df2[!, col])
        elseif eltype(df2[!, col]) <: AbstractFloat
            df2[!, col] = round.(df2[!, col], digits=2)
        end
    end
    show(stdout, df2; eltypes=false)
end
```
...
@printf "IDE (Direct)   = %.4f (SE = %.4f)\n" ide["estimate"] ide["std.error"]
@printf "IIE (Indirect) = %.4f (SE = %.4f)\n" iie["estimate"] iie["std.error"]
