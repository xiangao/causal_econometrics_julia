...
DataFrame(Estimand=["CDE(0) G-computation"], Value=[round(cde_gcomp, digits=4)])
```
...
DataFrame(Estimand=["CDE(0) IPW"], Value=[round(cde_ipw, digits=4)])
```
...
DataFrame(Estimand=["CDE(0) AIPW"], Value=[round(cde_aipw, digits=4)])
```
...
DataFrame(
    Effect = ["NDE", "NIE", "ATE"],
    Value  = round.([E_Y1M0 - E_Y0M0, E_Y1M1 - E_Y1M0, E_Y1M1 - E_Y0M0], digits=4)
)
```
...
DataFrame(
    Effect   = ["IDE (Direct)", "IIE (Indirect)", "ATE"],
    Estimate = round.([ide["estimate"], iie["estimate"], result.estimates["ate"]["estimate"]], digits=4),
    SE       = [round(ide["std.error"], digits=4), round(iie["std.error"], digits=4), missing]
)
```