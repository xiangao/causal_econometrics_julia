...
DataFrame(
    Estimand = ["Tau (bias-corrected)", "P-value (robust)"],
    Value    = [round(rdd_gpa.Estimate.tau_us[1], digits=4),
                round(rdd_gpa.pv.PValue[3], digits=6)]
)
```
...
DataFrame(
    Estimand = ["Tau (bias-corrected)", "P-value (robust)"],
    Value    = [round(rdd_house.Estimate.tau_us[1], digits=4),
                round(rdd_house.pv.PValue[3], digits=6)]
)
```
...
m_rdd = rdrobust(vet.home_ownership,
                 ...
                 c = 0.0)
DataFrame(
    Method   = ["Manual 2SLS", "RDRobust (FRDD)"],
    Tau      = [round(m_iv.beta[3], digits=4),
                round(m_rdd.Estimate.tau_us[1], digits=4)]
)
```
