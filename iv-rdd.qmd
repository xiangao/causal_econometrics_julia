...
using Vcov
using DataFrames
using StatsModels, StatsAPI, DataFramesMeta
```
...
show(stdout, MIME("text/plain"), StatsAPI.coeftable(lm_biased))

# Full OLS is good
println("\n── Full OLS (w + m) ─────────────────────────")
show(stdout, MIME("text/plain"), StatsAPI.coeftable(lm_full))
...
# not the true w.
println("\n── Manual 2SLS ──────────────────────────────")
show(stdout, MIME("text/plain"), StatsAPI.coeftable(tsls_manual))
```
...
println("Tau (bias-corrected) = ", round(rdd_gpa.Estimate.tau_us[1], digits=2))
println("P-value (robust)     = ", round(rdd_gpa.pv.PValue[3], digits=4))
```
...
println("Tau (bias-corrected) = ", round(rdd_house.Estimate.tau_us[1], digits=2))
println("P-value (robust)     = ", round(rdd_house.pv.PValue[3], digits=4))
```
...
show(stdout, MIME("text/plain"), StatsAPI.coeftable(m_iv))
```
...
m_rdd = rdrobust(vet.home_ownership,
                 ...
                 fuzzy = vet.vet_wwko,
                 c = 0.0)
println("Manual 2SLS     tau = ", round(m_iv.beta[3], digits=2))
println("RDRobust (FRDD) tau = ", round(m_rdd.Estimate.tau_us[1], digits=2))
```
