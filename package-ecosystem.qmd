...
| `CausalInference.jl` (local clone) | PC and FCI algorithms, Fisher-Z CI tests | Causal Discovery (both chapters) |
| `RecursiveCausalDiscovery.jl` (local clone) | RSL-D and L-MARVEL recursive discovery, F1 scoring | Causal Discovery (both chapters) |
...
| `ShiftShareIV.jl` | Bartik/shift-share IV, Rotemberg weights, BHJ shock-level collapse | Shift-share IV |
| `DidInterference.jl` | DiD with spatial interference between units (`eval: false`) | DiD |
| `Endid.jl` | Distributional difference-in-differences (`eval: false`) | Distributional effects |
| `TVHTE.jl` | Time-varying heterogeneous event-study effects (`eval: false`) | DiD |
