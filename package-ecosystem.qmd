...
| `ETWFE` | ETWFE helper layer, `emfx()` aggregation, and a local `mpdta`-style dataset helper | DiD |
...
The `ETWFE` package is deliberately small. Its main role in this book is ETWFE post-processing:
...
The exported `att_gt()` function is currently a placeholder API. For the implemented workflow in this book, use `Panelest.feols(...)` plus `ETWFE.emfx(...)`.
...
```bash
julia --project -e 'using CausalEstimate, CausalGraphs, Panelest, ETWFE, SynthDiD, RDRobust, Lavaan, Crumble'
```
