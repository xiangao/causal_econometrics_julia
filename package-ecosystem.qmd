...
| `CausalGraphs.jl` | DAG/ADMG construction, graph-based identification, Pearl-Shpitser ID, and estimator routing | Identification, DAGs/ADMGs |
| `NPCausal.jl` | Influence-function estimators for ATE, ATT, continuous treatment effects, IV LATE, bounds, and incremental propensity interventions | Estimation, Nonparametric |
...
`NPCausal.jl` provides the nonparametric causal estimators used in the estimation and nonparametric chapters. The core pattern is to pass the outcome, treatment, and covariate `DataFrame`, then let the package handle cross-fitting and nuisance estimation.
...
`CausalGraphs.jl` is the graph layer for the DAG/ADMG chapter. It constructs
