...
- **Render**: `quarto render` (uses `julia-_book_-1.12` kernel with sysimage)
- **Render one chapter**: `quarto render nonparametric.qmd`
- **Rebuild sysimage**: `julia --project build_sysimage.jl` (~20 min; do this when packages change)
...
- `_quarto.yml` — Quarto config; kernel = `julia-_book_-1.12`
- `book_sysimage.so` — Precompiled sysimage for fast rendering
...
- Must rebuild sysimage whenever custom packages change (paths in Manifest.toml)
