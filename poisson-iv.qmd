...
The CF procedure (Wooldridge 2002, 2014):

1. **First stage** — linear projection of `time` on `phone`, `frfam`, and fixed effects via `feols`.  The residual $\hat{v}_2 = \text{time} - \hat{\text{time}}$ satisfies $E(\mathbf{z}'\hat{v}_2)=0$ by construction.
2. **Second stage** — Poisson QMLE with $\hat{v}_2$ added as a regressor.  Including $\hat{v}_2$ inside $\exp(\cdot)$ absorbs the endogenous variation.

Because the first stage is a linear model and the true DGP has a linear first stage, the CF assumption $E[y_1 \mid \mathbf{z_1}, y_2, v_2] = \exp(\mathbf{z_1}\boldsymbol{\delta_1} + \alpha_1 y_2 + \rho v_2)$ holds exactly here.
...
We create a binary endogenous indicator `time_hi` (high time on site) from the same latent index.  The structural error $e$ drives both `time_hi` (via the latent probit) and `visits` (via the conditional mean):

```{julia}
#| echo: true
Random.seed!(42)

# Binary endogenous variable from the same latent index
time_hi = Int.(time .>= median(time))   # median split of the continuous time

# Poisson outcome — same true coefficient 0.8, endogeneity via e
...
df2 = DataFrame(visits=visits2, time_hi=time_hi, phone=phone,
                frfam=frfam, female=female, ad=fe_grp, e=e)

println("time_hi mean: $(round(mean(time_hi),digits=3))")
println("Cor(time_hi, e): $(round(cor(time_hi, e), digits=3))  (endogeneity)")
```
...
println()
println("Note: CF approaches reduce naive bias substantially.")
println("Exact recovery of 0.8 is not guaranteed with binary EEV,")
println("as the CF assumption holds only approximately in this case.")
```
